...
var _defaultIndexer = function(item, options) {
  ...
  if (item.fields) {
    if (item.fields._parent) {
      index.index._parent = item.fields._parent;
    }
    if (item.fields._routing) {
      index.index._routing = item.fields._routing;
    }
    if (item.fields._timestamp) {
      index.index._timestamp = item.fields._timestamp;
    }
  }
